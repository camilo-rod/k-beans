...
""")
    ...
    # Procesar si hay foto nueva
    if foto_bytes is not None and foto_source != st.session_state.get("_foto_id_procesado"):
        st.session_state["_foto_id_procesado"] = foto_source
        for key in ("foto_feats", "img_contorno", "cluster", "new_pca", "resultado"):
            st.session_state.pop(key, None)

        with st.spinner("Analizando imagen…"):
            feats, img_contorno = extraer_features(foto_bytes)
    ...
    if foto_bytes is None:
        for key in ("_foto_id_procesado", "foto_feats", "img_contorno",
                    ...
                    "_raw_bytes", "_raw_source", "_raw_source_tab"):
            st.session_state.pop(key, None)
    ...
    if tiene_foto:
        ...
        # Toggle ajuste manual — ANTES del botón predecir
